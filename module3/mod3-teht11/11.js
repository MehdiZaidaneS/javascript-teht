...
for (let i = 0; i<picArray.length; i++){
  ...
  //Adding childs of figure
  figure.appendChild(img)
  figure.appendChild(figcaption)
  
  //Adding childs of article
  article.appendChild(h2)
  article.appendChild(figure)
  article.appendChild(p)
  
  //Adding article as child of section
  section.appendChild(article)

  
  // Function that sets img.src and opens the dialog.
  article.addEventListener('click', function(evt){
    imgDialog.src = `${picArray[i].image.large}`;
    dialog.showModal()
  })
  
  //Funciont that close the dialog.
